...
public class SvgPathParser {
  ...
  public static Path svgPathStr2SwtPath( Display display, String aPathStr ) {
    Path path = new Path( display );
    if( aPathStr == null || aPathStr.isBlank() ) {
      return path;
    }

    // Шаг 1: разобрать строку в список промежуточных команд
    List<Cmd> cmds = new Tokenizer( aPathStr.trim() ).tokenize();
