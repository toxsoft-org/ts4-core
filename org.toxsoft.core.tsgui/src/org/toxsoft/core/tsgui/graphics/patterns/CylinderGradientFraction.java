...
/**
 * Фракция цилиндрического градиента.
 * <p>
 *
 * @author vs
 */
public class CylinderGradientFraction
   ...
   */
  public CylinderGradientFraction( double aStartValue, RGBA aStartRgb, double aEndValue, RGBA aEndRgb ) {
    super( 1.8 * aStartValue - 180, 1.8 * aEndValue - 180, aStartRgb, aEndRgb );
    // nop
  }
  ...
  public RGBA calcRgb( double aValue ) {
    ...
    // RGB rgb = new RGB( Math.abs( red ), Math.abs( g ), Math.abs( b ) );
  ...
  protected IGradientFraction create( double aStartValue, double aEndValue, RGBA aStartRGB, RGBA aEndRGB ) {
    return new CylinderGradientFraction( aStartValue, aStartRGB, aEndValue, aEndRGB );
  }
}
