 ...
 * @author vs
 */
public class PanelLinearGradientSelectorEx
    extends TsPanel {
    ...
    PaintListener paintListener = aE -> {
      ...
      if( pattern != null ) {
        ...
        if( p != null ) {
          ...
          aE.gc.fillRectangle( 0, 0, getSize().x, getSize().y );
          p.dispose();
        }
      }
    };
  ...
  IGenericChangeListener changeListener = aSource -> {
    ...
    IList<Pair<Double, RGBA>> fractions = fractionsPanel.fractions();
    LinearGradientInfoEx gi = new LinearGradientInfoEx( centerX, centerY, fractions );
    pattern = gi.createGradient( tsContext() );
    resultPanel.redraw();
  };
  ...
  PanelLinearGradientSelectorEx( Composite aParent, ITsGuiContext aContext ) {
  ...
  public IGradientInfo gradientInfo() {
    return new LinearGradientInfoEx( centerX, centerY, fractionsPanel.fractions() );
  }
