 ...
 * @author vs
 */
public class PanelConicalGradientSelector
  ...
  IGenericChangeListener changeListener = aSource -> {
    ...
    ConicalGradientInfo gi = new ConicalGradientInfo( fractions, angle );
  ...
  PanelConicalGradientSelector( Composite aParent, ITsGuiContext aContext ) {
  ...
  public IGradientInfo gradientInfo() {
    return new ConicalGradientInfo( fractionsPanel.fractions(), angle );
  }
  ...
  public void setGradientInfo( IGradientInfo aInfo ) {
    TsIllegalArgumentRtException.checkFalse( aInfo.gradientType() == EGradientType.CONICAL );
    ConicalGradientInfo info = (ConicalGradientInfo)aInfo;
    // IListEdit<Pair<Double, RGBA>> pairs = new ElemArrayList<>();
    // for( Pair<Double, RGBA> p : info.fractions() ) {
    // Pair<Double, RGBA> pair = new Pair<>( (100 * p.left()) / (Math.PI * 2), p.right() );
    // pairs.add( pair );
    // }
    // fractionsPanel.setFractions( pairs );
