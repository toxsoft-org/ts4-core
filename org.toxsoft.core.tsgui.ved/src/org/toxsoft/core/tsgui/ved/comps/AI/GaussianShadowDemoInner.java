 ...
 * </ol>
 */
public class GaussianShadowDemoInner {
  ...
  public static void main( String[] args ) {
    new GaussianShadowDemo2Inner().run();
  }
  ...
  private static Path buildDemoPath( Device device, int W, int H ) {
  ...
  private ShadowCache buildShadowCache( Device device, int canvasW, int canvasH ) {
    ...
    float[] blurred = boxBlurApprox( invertedMask, imgW, imgH, shadowRadius );
   ...
   * радиуса.
   */
  private float[] boxBlurApprox( float[] src, int W, int H, int radius ) {
