 ...
 * текстуры.
 */
public class FlatSteelRingSWT {

  // ── Геометрия ────────────────────────────────────────────────────
  private static int outerRadius = 140;
  private static int ringThick   = 56;
  ...
  public static void main( String[] args ) {
    ...
    shell.setText( "Flat Steel Ring — SWT" );
  ...
  private static void paintScene( GC gc, Canvas canvas ) {
    ...
    if( dirty ) {
      cachedData = useTexture && textureData != null ? generateTextureRing( size, size, outerRadius, innerR )
          : generateConicRing( size, size, outerRadius, innerR );
      dirty = false;
    }
  ...
  // ════════════════════════════════════════════════════════════════
  private static ImageData generateConicRing( int w, int h, int outerR, int innerR ) {
    ...
    for( int y = 0; y < h; y++ ) {
      for( int x = 0; x < w; x++ ) {
        ...
        // ── Тонкое кольцевое затемнение краёв (rim) ─────────
        float t = (dist - innerR) / (outerR - innerR); // 0..1
        float rim = 1f - (float)Math.pow( Math.abs( t - 0.5f ) * 2f, 3f ) * 0.25f;
        steelVal *= rim;
  ...
  // Б) ТЕКСТУРА
  // ════════════════════════════════════════════════════════════════
  private static ImageData generateTextureRing( int w, int h, int outerR, int innerR ) {
    ...
    for( int y = 0; y < h; y++ ) {
      for( int x = 0; x < w; x++ ) {
        ...
        // Тонкое затемнение краёв кольца
