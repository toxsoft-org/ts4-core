 ...
 */
class VedScreen
    implements IVedScreen, ITsGuiContextable, IVedScreenPaintingManager {
   ...
   * @param aVedEnv {@link VedEnvironmentEdit} - the VED environment
   * @throws TsNullArgumentRtException any argument = <code>null</code>
   */
  public VedScreen( Canvas aCanvas, VedEnvironmentEdit aVedEnv ) {
    TsNullArgumentRtException.checkNulls( aCanvas, aCanvas );
    theCanvas = aCanvas;
    vedEnv = aVedEnv;
    vedEnv.doc().components().items().addCollectionChangeListener( componentsListChangeListener );
  ...
  void refreshViewsListFromCompnentsList() {
    IStridablesListEdit<IVedComponent> compsList = new StridablesList<>( vedEnv.doc().components().items() );
