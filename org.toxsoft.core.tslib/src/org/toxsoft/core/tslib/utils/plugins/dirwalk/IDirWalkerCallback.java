 ...
 * началом обработки файлов список сортируется по именам файлов, т.е. processFile() вызывается по порядку имен файлов</li>
 ...
 * <li>После processFinish() обходчик начинает рекурсивный обход подкаталогов и повторяется вышеописанная процедура.</li>
 * <li>После "глубокого" прохода по директорию (уже и с поддиректориями) вызывается {@link #dirExit(File) dirExit()}.</li>
...
public interface IDirWalkerCallback {
   ...
   *          задана)
   * @return boolean <li>true - перед вызовом processStart() список файлов для обработки будет обновлен <li>false - во
   *         избежание лишней работы обходчик использует список, сформированный к моменту вызова dirEnter().
   * @throws DirWalkerCanceledException если реализация выбросила данное исключение, то обход дерева будет прекращен.
   */
