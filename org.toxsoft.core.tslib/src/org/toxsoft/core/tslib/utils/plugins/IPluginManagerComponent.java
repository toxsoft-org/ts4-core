 ...
 * <p>
 * Компонента отслеживает перечень плагинов заданного типа ( {@link IPluginsHardConstants#PLUGIN_TYPE_ID}) в директориях
 * размещения aDirs (метода {@link #init(IOptionSet, Object...)}). Каждые
 * {@link IPluginsHardConstants#DIR_CHECK_INTERVAL} миллисекунд компонента проверят наличие изменений в составе и
 * версиях файлов в заданных директориях, и обнаружив изменения, вызывает слушателя, заданного методом
 * {@link #setPluginsChangeListener(IPluginsChangeListener)}. Кроме того, поддерживает в актуальном состоянии список
 * найденных плагинов (метод {@link #listPlugins()}).
 * <p>
 * <p>
 * Компонента должна быть инициализирована до и завершена после использовани (реализует интерфейс
 * <code>IInitializable</code>} . Кроме того, она выполняет работу в режиме кооперативной многозадачности
 * ({@link ICooperativeMultiTaskable}), то есть, не запускат свои поток выполнения. Надо не забыть, достаточно часто
 * вызывать {@link #doJob()}.
 * <p>
...
public interface IPluginManagerComponent
   ...
   *
   * @param aOps {@link IOptionSet} - параметры из перечня {@link IPluginsHardConstants}
