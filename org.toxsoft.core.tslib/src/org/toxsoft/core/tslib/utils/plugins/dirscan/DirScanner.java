...
import org.toxsoft.core.tslib.utils.plugins.dirwalk.*;
...
public class DirScanner {
   ...
   */
  public static final FileFilter JAR_FILTER = TsFileFilter.ofFileExt( JAR_EXT );
  ...
  public DirScanner( File aScanPath, FileFilter aFilter, boolean aIncludeSubDirs,
    ...
    TsFileUtils.checkDirReadable( aScanPath );
    if( aFilter != null ) {
      scanFilter = aFilter;
    }
    else {
      scanFilter = TsFileFilter.FF_FILES_HIDDEN;
    }
    scanPath = aScanPath;
  ...
  private class ScannerCb
    ...
    public void processFile( File aDir, File aFile )
      ...
      if( dsCallback != null ) {
        ...
        dsCallback.step( done, ITsResources.MSG_STEP_PROCESSING_FILE, fileObjName );
      }
      ++doneCount;
    }
  ...
  public IDirScanResult scan( IDirScanCallback aCallback ) {
    ...
    if( !scanningOn ) {
      ...
      try {
        TsFileUtils.checkDirReadable( scanPath );
