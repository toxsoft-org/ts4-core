...
class M5Domain
  ...
  private final IStridablesListEdit<IDataDef>    dataDefs = new StridablesList<>();
  ...
  @Override
  public IStridablesList<IDataDef> dataDefs() {
    if( parent == null ) {
      return dataDefs;
    }
    IStridablesListEdit<IDataDef> ll = new StridablesList<>();
    ll.addAll( parent.dataDefs() );
    ll.addAll( dataDefs );
    return ll;
  }

  @Override
  public IDataDef getDataDef( String aDataId ) {
    IDataDef dd = dataDefs.findByKey( aDataId );
    if( dd == null && parent != null ) {
      dd = parent.getDataDef( aDataId );
    }
    return dd;
  }
  ...
  public <T> IM5Model<T> getModel( String aModelId, Class<T> aEntityClass ) {
    ...
    if( foundModel == null ) {
      throw new TsItemNotFoundRtException( FMT_ERR_NO_MODEL_ID_IN_DOMAIN, aModelId );
    }
