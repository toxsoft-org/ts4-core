...
/**
 * Sorted {@link IListBasicEdit} implementation with external comparator.
 *
 * @author hazard157
 * @param <E> - the type of elements in this collection
 */
public class SortedElemLinkedBundleListEx<E>
   ...
   *
   * @param aComparator {@link Comparable} - elements comparator
   ...
   * @throws TsNullArgumentRtException any argument = <code>null</code>
   */
  public SortedElemLinkedBundleListEx( Comparator<E> aComparator ) {
    this( aComparator, DEFAULT_BUNDLE_CAPACITY, true );
  }
  ...
  public int indexOf( E aElem ) {
    ...
    int index = 0;
    do {
      if( comparator.compare( aElem, b.lastValue() ) <= 0 ) { // aValue <= b.lastValue()
        for( int i = 0; i < b.count; i++ ) {
          if( b.elem( i ).equals( aElem ) ) { // b.elems[i] == aValue
            return i + index;
          }
          if( comparator.compare( aElem, b.elem( i ) ) < 0 ) { // aValue < b.elems[i]
            return -1;
          }
        }
        return -1;
  ...
  public int add( E aElem ) {
    ...
    do {
      if( comparator.compare( aElem, b.lastValue() ) <= 0 ) { // aValue <= b.lastValue()
        ...
        ++size;
        // вставим aValue в место i+1
        if( b.isFull() ) {
          Bundle bNext = b.next;
          if( bNext == null || bNext.isFull() ) { // need to create new bundle
